            ...
            messages = [
                {
                    ...
                    "content": [
                        ...
                        {
                            "type": "text",
                            "text": f"""这是一张图片，图片上文部分为"{img_context.get("pre_text")}"，
                                下文部分为"{img_context.get("post_text")}"，请用中文简要总结这张图片的摘要,字数在50字以内。""",
                        },
                    ],
                },
            ]
            res = llm.invoke(input=messages)
            img_summary_list.append(
                {"img_name": img_name, "img_path": img_path, "summary": res.content}
            )
        logger.info(img_summary_list)
